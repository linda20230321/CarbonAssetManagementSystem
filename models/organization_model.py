...
class EnergyDataDAO:
    ...
    @staticmethod
    def get_by_org_and_period(org_id, period_type, period_year, period_month=None, period_quarter=None):
        """按组织和周期条件查询能耗数据"""
        with get_db() as conn:
            cursor = conn.cursor()
            if period_type == '月度' and period_month is not None:
                cursor.execute('''
                    SELECT id, org_id, org_name, period_type, period_year, period_month,
                           period_quarter, period_start, period_end, data_json, total_emission,
                           created_at
                    FROM energy_data 
                    WHERE org_id = ? AND period_type = ? AND period_year = ? AND period_month = ?
                    ORDER BY created_at DESC
                ''', (org_id, period_type, period_year, period_month))
            elif period_type == '季度' and period_quarter is not None:
                cursor.execute('''
                    SELECT id, org_id, org_name, period_type, period_year, period_month,
                           period_quarter, period_start, period_end, data_json, total_emission,
                           created_at
                    FROM energy_data 
                    WHERE org_id = ? AND period_type = ? AND period_year = ? AND period_quarter = ?
                    ORDER BY created_at DESC
                ''', (org_id, period_type, period_year, period_quarter))
            else:
                cursor.execute('''
                    SELECT id, org_id, org_name, period_type, period_year, period_month,
                           period_quarter, period_start, period_end, data_json, total_emission,
                           created_at
                    FROM energy_data 
                    WHERE org_id = ? AND period_type = ? AND period_year = ?
                    ORDER BY created_at DESC
                ''', (org_id, period_type, period_year))
            rows = cursor.fetchall()
            result = []
            for row in rows:
                item = dict(row)
    ...
    def delete(id):
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('DELETE FROM energy_data WHERE id = ?', (id,))
            conn.commit()
            return cursor.rowcount > 0
    ...
    def update(id, data):
        ...
        with get_db() as conn:
            ...
            cursor.execute('''
                UPDATE energy_data 
                SET org_id = ?, org_name = ?, period_type = ?, period_year = ?,
                    period_month = ?, period_quarter = ?, period_start = ?, period_end = ?,
                    data_json = ?, total_emission = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ''', (
                data.get('org_id'),
                data.get('org_name'),
