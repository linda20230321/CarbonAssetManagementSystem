...
function switchOrgTab(tab) {
    ...
    if (tab === 'detail') {
        if (!currentOrgId) {
            ...
            // 从数据库加载最新记录的 details 并渲染表格
            loadDetailFromDB(currentOrgId);
        }
    }
    if (tab === 'trend') {
        if (!currentOrgId) {
...
function queryResults() {
    ...
    // 构建查询参数
    var periodType = (document.getElementById('resultPeriodType') || {}).value || '月度';
    var periodYear = parseInt((document.getElementById('resultYear') || {}).value || 2024);
    var periodMonth = parseInt((document.getElementById('resultMonth') || {}).value);
    var periodQuarter = (document.getElementById('resultQuarter') || {}).value || null;

    var queryParams = '?period_type=' + encodeURIComponent(periodType) + '&period_year=' + periodYear;
    if (periodType === '月度' && periodMonth) {
        queryParams += '&period_month=' + periodMonth;
    }
    if (periodType === '季度' && periodQuarter) {
        queryParams += '&period_quarter=' + encodeURIComponent(periodQuarter);
    }

    fetch('/api/energy_data/' + currentOrgId + '/query' + queryParams)
    ...
    .then(function(data) {
        if (data && data.length > 0) {
            ...
            // 按范围汇总
            var range1Emission = 0, range2Emission = 0, range3Emission = 0;
            if (Array.isArray(details)) {
                for (var i = 0; i < details.length; i++) {
                    var d = details[i];
                    if (d.range === '范围1') range1Emission += (d.emission || 0);
                    else if (d.range === '范围2') range2Emission += (d.emission || 0);
                    else if (d.range === '范围3') range3Emission += (d.emission || 0);
                }
            }

            document.getElementById('resultValue1').textContent = range1Emission.toFixed(2);
            document.getElementById('resultValue2').textContent = range2Emission.toFixed(2);
            document.getElementById('resultValue3').textContent = range3Emission.toFixed(2);
            document.getElementById('resultTotal').textContent = total.toFixed(2);

            calculationResults = {
                org_id: currentOrgId,
                org_name: latest.org_name || currentOrgName,
                details: details,
                total: total
            };

            alert('查询成功！找到 ' + data.length + ' 条记录，显示最新结果。');
        } else {
...
function loadResultsFromDB(orgId) {
    ...
    .then(function(data) {
        if (data && data.length > 0) {
            ...
            // 按范围1/2/3分别汇总排放量
            var range1Emission = 0;
            var range2Emission = 0;
            var range3Emission = 0;

            if (Array.isArray(details)) {
                for (var i = 0; i < details.length; i++) {
                    var d = details[i];
                    if (d.range === '范围1') {
                        range1Emission += (d.emission || 0);
                    } else if (d.range === '范围2') {
                        range2Emission += (d.emission || 0);
                    } else if (d.range === '范围3') {
                        range3Emission += (d.emission || 0);
                    }
                }
            }
            ...
            console.log('✅ 从数据库加载结果: 范围1=' + range1Emission.toFixed(2) + ', 范围2=' + range2Emission.toFixed(2) + ', 范围3=' + range3Emission.toFixed(2) + ', 总计=' + total.toFixed(2));
        } else {
            document.getElementById('resultValue1').textContent = '0';
            document.getElementById('resultValue2').textContent = '0';
            document.getElementById('resultValue3').textContent = '0';
            document.getElementById('resultTotal').textContent = '0';
            console.log('⚠️ 数据库中无能耗数据');
        }
    })
...
function loadDetailFromDB(orgId) {
    if (!orgId) orgId = currentOrgId;
    if (!orgId) return;
    ...
    .then(function(data) {
        if (data && data.length > 0) {
            ...
            updateDetailPage(details, total);
            console.log('✅ 详情页从数据库加载了 ' + details.length + ' 条排放源数据');
        } else {
            updateDetailPage([], 0);
            console.log('⚠️ 数据库中无详情数据');
        }
    })
    .catch(function(e) {
        console.log('加载详情数据失败:', e);
        updateDetailPage([], 0);
    });
}
...
function drawTrendChart() {
    ...
    canvas.width = (container.clientWidth - 40) || 700;
    canvas.height = 350;
    ...
    if (!currentOrgId) {
        ...
        ctx.font = '14px Arial';
        ctx.fillText('请先选择组织', 20, 60);
        return;
    }

    // 从数据库加载当前组织的所有能耗数据
    fetch('/api/energy_data/' + currentOrgId)
    .then(function(response) { return response.json(); })
    .then(function(data) {
        if (!data || data.length === 0) {
            ctx.fillStyle = '#95a5a6';
            ctx.font = '14px Arial';
            ctx.fillText('暂无趋势数据，请先进行碳核算', 20, 60);
            return;
        }

        // 按时间排序（period_year, period_month）
        ...
        for (var i = 0; i < data.length; i++) {
            var record = data[i];
            var label = record.period_year + '-' + (record.period_month < 10 ? '0' : '') + record.period_month;
            labels.push(label);

            var details = typeof record.data_json === 'string' ? JSON.parse(record.data_json) : (record.data_json || []);
            var r1 = 0, r2 = 0, r3 = 0;
            if (Array.isArray(details)) {
                for (var j = 0; j < details.length; j++) {
                    var d = details[j];
                    if (d.range === '范围1') r1 += (d.emission || 0);
                    else if (d.range === '范围2') r2 += (d.emission || 0);
                    else if (d.range === '范围3') r3 += (d.emission || 0);
                }
            }
            range1Data.push(r1);
            range2Data.push(r2);
            range3Data.push(r3);
            totalData.push(record.total_emission || (r1 + r2 + r3));
        }

        // 绘制图表
        _renderTrendChart(ctx, canvas, labels, range1Data, range2Data, range3Data, totalData);
    })
    .catch(function(e) {
        ctx.fillStyle = '#e74c3c';
        ctx.font = '14px Arial';
        ctx.fillText('加载趋势数据失败: ' + e, 20, 60);
    });
}

function _renderTrendChart(ctx, canvas, labels, range1Data, range2Data, range3Data, totalData) {
    var W = canvas.width;
    var H = canvas.height;
    var padding = { top: 40, right: 30, bottom: 60, left: 70 };
    var chartW = W - padding.left - padding.right;
    var chartH = H - padding.top - padding.bottom;

    // 计算Y轴范围
    var allValues = totalData.concat(range1Data, range2Data, range3Data);
    ...
    for (var i = 0; i <= gridLines; i++) {
        ...
        var val = minVal + (i / gridLines) * (maxVal - minVal);
        ctx.beginPath();
    ...
    for (var i = 0; i < labels.length; i++) {
        if (i % step === 0 || i === labels.length - 1) {
            var x = padding.left + (i / (labels.length - 1)) * chartW;
    ...
    ctx.font = '12px Arial';
    ctx.textAlign = 'center';
    ctx.fillText('月份', padding.left + chartW / 2, H - 8);
    ...
    function drawLine(dataPoints, color, lineWidth) {
        ...
        for (var i = 0; i < dataPoints.length; i++) {
            ...
            var y = padding.top + chartH - ((dataPoints[i] - minVal) / (maxVal - minVal)) * chartH;
    ...
    // 绘制数据点函数
    function drawPoints(dataPoints, color) {
        for (var i = 0; i < dataPoints.length; i++) {
            var x = padding.left + (i / (dataPoints.length - 1)) * chartW;
            var y = padding.top + chartH - ((dataPoints[i] - minVal) / (maxVal - minVal)) * chartH;
            ctx.beginPath();
    ...
    // 绘制各条折线（先绘制总量，再绘制范围1/2/3）
    drawLine(totalData, '#2c3e50', 2.5);
    drawPoints(totalData, '#2c3e50');

    drawLine(range1Data, '#e74c3c', 2);
    drawPoints(range1Data, '#e74c3c');

    drawLine(range2Data, '#3498db', 2);
    drawPoints(range2Data, '#3498db');

    drawLine(range3Data, '#2ecc71', 2);
    drawPoints(range3Data, '#2ecc71');

    // 绘制图例
    var legendX = padding.left + 10;
    var legendY = padding.top + 8;
    var legendItems = [
        { color: '#2c3e50', label: '总排放量' },
        { color: '#e74c3c', label: '范围1' },
        { color: '#3498db', label: '范围2' },
        { color: '#2ecc71', label: '范围3' }
    ];
    ...
    ctx.textAlign = 'center';
    ctx.fillText('碳排放趋势图 - ' + currentOrgName, W / 2, 20);
}
...
function calculateEmissions() {
    ...
    // 自动将核算结果保存到数据库
    var periodType = document.getElementById('inputPeriod').value;
    var periodYear = parseInt((document.getElementById('inputYear') || {}).value || 2024);
    var periodMonth = parseInt((document.getElementById('inputMonth') || {}).value || 1);
    var periodQuarter = (document.getElementById('inputQuarter') || {}).value || null;
    ...
    .then(function(res) {
        if (res.success) {
            console.log('✅ 核算结果已保存到数据库');
        } else {
            console.log('⚠️ 保存核算结果失败:', res.error || res);
        }
    })
    .catch(function(e) {
        console.log('⚠️ 保存核算结果异常:', e);
    });
}
...
function saveMethod() {
    ...
    if (editIndex !== '') {
        ...
        var methodId = calculationMethodsData[idx] ? calculationMethodsData[idx].id : null;
        if (methodId) {
            fetch('/api/calculation_methods/' + methodId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
            ...
            calculationMethodsData[idx] = Object.assign({}, calculationMethodsData[idx], methodData);
            alert('✅ 核算方法已更新（本地）');
            renderCalculationMethods();
        }
    } else {
...
function deleteMethod() {
    ...
    idx = parseInt(idx) - 1;
    if (idx < 0 || idx >= calculationMethodsData.length) { alert('序号无效'); return; }
    var method = calculationMethodsData[idx];
    if (confirm('确定要删除 "' + method.method_name + '" 吗？')) {
